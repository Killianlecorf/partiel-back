...
export class Migration20250206102918 extends Migration {
  ...
  override async up(): Promise<void> {
    ...
    this.addSql(`create table "user" ("id" serial primary key, "name" varchar(255) not null, "email" varchar(255) not null, "is_admin" boolean not null, "created_at" timestamptz not null, "update_at" timestamptz not null);`);

    this.addSql(`create table "order" ("id" serial primary key, "name" varchar(255) not null, "statut" varchar(255) not null, "created_at" timestamptz not null, "update_at" timestamptz not null, "user_id" int not null);`);

    this.addSql(`create table "order_item" ("id" serial primary key, "product_id" int not null, "order_id" int not null, "quantity" int not null);`);
